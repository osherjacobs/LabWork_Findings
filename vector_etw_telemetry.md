...
**Note on comparability:** The binary used in Vector 4 was a minimal ~17-line C# implementation, unobfuscated, with a standard PE structure and visible import table. The memory walker used in this chain is substantially more complex and was passed through ConfuserEx prior to deployment — producing a zeroed IMPHASH and absent PE metadata as documented in the primary writeup. The scan surface presented to the engine differs between the two sessions in ways that are not reducible to implementation size: obfuscation approach, import table structure, and PE metadata presence are all variables. The throughput degradation observed in April therefore occurred against the simpler, unobfuscated binary. Whether obfuscation state or behavioral surface — rather than binary complexity per se — is the relevant variable for runtime enforcement behavior remains untested in a controlled side-by-side.
