...
EPA is the enforcement mechanism for CBT — they are not separate controls. The failure point is the absence of binding between the relayed Kerberos service ticket and the TLS channel characteristics. The service accepts the ticket without validating channel context.

| Control | NTLM relay | Kerberos relay |
|---------|------------|----------------|
| HTTPS alone | ❌ | ❌ |
| EPA = Require (no CBT validation for Kerberos) | ✅ | ❌ |
| EPA = Require (with CBT validation for Kerberos) | ✅ | ✅ |

EPA can protect Kerberos when CBT is present and validated. Many HTTP services — including ADCS Web Enrollment — do not enforce CBT validation for Kerberos service tickets, leaving a relay surface even when EPA is set to Require.

HTTPS is a prerequisite for CBT, not a substitute for it. HTTPS alone does not block NTLM or Kerberos relay — it enables the channel binding material that CBT enforcement depends on.

### Silent Kerberos traffic — no user interaction required

Enterprises generate Kerberos traffic constantly from background sources:

- Explorer network discovery
- WebDAV probes
- Scheduled tasks
- Service health checks
- WinRM pre-authentication
- Browser proxy auto-detection (WPAD)
- DFS referrals
- SMB session refresh

Relay viability is determined by network position, not user behavior. An attacker does not need to socially engineer authentication — the traffic already exists.
