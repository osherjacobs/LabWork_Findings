...
> This is a detection engineering document. The nature of the payload is intentionally omitted. The focus is on what the SIEM sees — not on how to build offensive tooling.

---

## Threat Model

This is not the "external attacker who somehow obtained credentials" scenario. The threat model here is the **rotten apple admin** — a privileged insider with interactive access, knowledge of the environment, and the ability to modify local endpoint controls before executing anything.

This matters because:

- A compromised admin can disable Defender cloud submission via the UI or registry before dropping a payload — no prompt, no interruption
- Local logs, local Task Scheduler visibility, and local Defender configuration are all within reach of a privileged insider
- **What they cannot easily reach is logs already shipped to an external SIEM**

This is precisely why external log shipping exists as a control. Sysmon and Windows Security logs land in ELK before the shell connects. The EID 4698 with full task XML, the child process chain with ParentCommandLine visible — all of it is off the box and immutable from the attacker's perspective by the time they run their first command.

The detection rules in this document fire on that external telemetry. The attacker controlled the endpoint. They did not control the log destination.
...
- A payload developed on a separate machine and ready to deploy (`ld.exe` + `ld.ps1`)
...
An attacker with valid domain admin credentials and network access to the target can drop files directly via SMB — no exploit required. `C:\Windows\Tasks` is the target: a well-documented AppLocker bypass path writable by admins.
...
Set hidden + system attributes to defeat casual `dir` and Explorer inspection.

```bash
impacket-wmiexec 'administrator:<PASSWORD>@192.168.1.4' \
  'attrib +h +s C:\Windows\Tasks\ld.exe && attrib +h +s C:\Windows\Tasks\ld.ps1'
```
...
Remote task creation over TSCH (MS-TSCH RPC). Task runs as `LocalSystem`, hidden from Task Scheduler UI.
...
| Defender inspection | Sysmon EID 1 | `notepad.exe` opening `ld.ps1` from `C:\Windows\Tasks\` — Defender's own analysis process |
...
| Tool | Purpose |
|---|---|
| goexec v0.3.0 | Remote scheduled task registration via MS-TSCH |
| impacket-wmiexec | Remote command execution for attrib |
