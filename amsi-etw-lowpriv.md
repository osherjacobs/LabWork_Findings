...
The PowerShell ETW provider (`PSEtwLogProvider`) exposes its registration handle as a private field accessible via .NET reflection. Zeroing this handle in-process causes the provider to emit no events for the remainder of the session, while continuing to report itself as active (`m_enabled=1`).

The patch targets `m_regHandle` on the `etwProvider` field of `PSEtwLogProvider` via `NonPublic,Static` and `NonPublic,Instance` binding flags. No native calls, no ntdll patching, no kernel interaction. Pure managed reflection against a loaded assembly.

Result: EID 4104 scriptblock logging stops for the session. The provider appears active but is effectively deaf.

---

## AMSI Bypass

Defender's AMSI signatures match on known type and field name strings — including across naive string concatenation, which is partially evaluated before the scan surface. A construction technique that defers string assembly until runtime avoids this scan surface entirely, as the buffer sees only the construction call and its arguments, never the assembled target string.

The key constraint discovered in lab: evil-winrm wraps each submission in `Invoke-Expression`, causing AMSI to scan each block independently. Multi-line submissions containing recognizable patterns fire even after ETW is patched, because AMSI operates independently of ETW. Single-line submission of each reflection step avoids this.

The `BindingFlags` parameter must be cast to `[System.Reflection.BindingFlags]` explicitly — passing a plain string returns null from `GetField` and silently fails.

---

## Execution Chain

With ETW patched and AMSI bypassed, a reconnaissance tool (PowerView) was loaded over HTTP from a Kali host and a reverse shell was established. The shell process inherits the patched runtime — both controls remain degraded for all commands executed within the session.

Implementation details are intentionally omitted. The mechanism is well-documented in prior public research. The contribution here is the privilege context and telemetry analysis, not the bypass primitives.
...
## Why Runtime String Assembly Evades Signatures

Defender's AMSI signatures operate on the string content of scriptblocks as presented to the scan buffer. Techniques that defer construction of signatured strings until after the scan window present only the construction scaffolding to the scanner — the assembled value never appears as a literal. This is not a new technique. It is a demonstration that Defender's current signature set does not cover this construction pattern for known AMSI targets as of May 2026.

---

## Detection Surface

PowerShell scriptblock telemetry (EID 4104) is the primary content-visibility mechanism. Once the in-process provider handle is zeroed, 4104 stops firing for all subsequent commands in the session. What remains:

- **Authentication:** EID 4776 credential validation, EID 4624 logon type 3 — both fire before any bypass code executes
- **Process:** `wsmprovhost.exe → powershell.exe` ancestry — logged at session start, bypass-agnostic
- **Network:** HTTP GET for payload, outbound TCP to C2 — visible in network telemetry and Sysmon EID 3 if deployed
- **LDAP:** EID 4662 burst from enumeration — attributed to machine account, requires session correlation to link to the user

**What disappeared was PowerShell content visibility. Host, network, and authentication observability remained intact.**

**This detection outlives the technique:** the WinRM logon and process ancestry are logged before any bypass executes. A detection rule correlating `wsmprovhost.exe → powershell.exe` with subsequent outbound TCP and absent 4104 is bypass-agnostic.
...
|---|---|---|
| AMSI | Bypassed | Runtime string assembly evades signature scan surface |
| PowerShell ETW / EID 4104 | Bypassed | In-process provider handle zeroed via reflection |
| Defender RTP | Did not prevent | No PE drop, no shellcode |
| Privilege requirement | None | Reflection operates on current process assemblies |
...
The privilege boundary does not protect AMSI or PowerShell scriptblock logging. Any authenticated user granted interactive PowerShell access can interfere with these controls in-process without local admin rights. The realistic assumed-breach posture should treat both controls as degraded once an attacker has interactive PowerShell — regardless of privilege level.
...
All commands executed from reverse shell as `lab2019\lowpriv`. Defender RTP confirmed enabled prior to session. No commands blocked.

### Kerberoastable Accounts

`bob.harris` — `MSSQLSvc/fake.lab2019.local:1433`. `msds-supportedencryptiontypes` = 16 (AES256 only — RC4 disabled domain-wide).
...
`WIN-JOCP945SK51.lab2019.local` (DC only — expected, not exploitable from lowpriv without a coercion primitive).

### DCSync ACL

Only expected principals hold replication rights — Enterprise RODCs, Domain Controllers, Administrators, Enterprise DCs. No rogue DCSync delegation. Clean.
...
| Event | Detail |
|---|---|
| EID 4776 | Credential validation — lowpriv, NTLM. Fires before any code executes. Earliest indicator. |
| EID 4624 | Logon type 3, NTLM V2, null Kerberos GUID. WinRM session anchor. |

**Post-bypass silence (absent):**
- No EID 4104 scriptblock logging after provider handle zeroed
- No Defender alert for bypass, tool load, or reverse shell

**The attribution gap:**
EID 4662 burst (LDAP enumeration) attributed to DC machine account, not lowpriv. Without session correlation it is indistinguishable from normal DC self-enumeration.

**Bypass-agnostic detection pattern:**
```
EID 4776 (lowpriv, NTLM)
→ EID 4624 (logon type 3)
→ EID 4662 burst
→ EID 4104 absent
```

The gap where 4104 should be is the signal. A detection rule correlating `4624 type 3 from non-admin source + subsequent 4662 volume + no 4104 within session window` catches this regardless of bypass technique.

### Additional Findings

- `MachineAccountQuota` = 10 — RBCD attack viable if GenericWrite target identified
- Non-standard `GenericWrite` delegation on a domain user — targeted Kerberoast or shadow credentials path
- Account Operators holds `GenericAll` on all computers in `CN=Computers` — escalation path if membership obtained
